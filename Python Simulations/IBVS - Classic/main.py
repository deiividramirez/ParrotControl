...
I3 = np.eye(3)
kp = 3
L = ImageJacobian(DESIRED_DATA.inNormalPlane.T, np.ones((POINTS.shape[0], 1)), 1)
...
for t in range(time.shape[0]):
    ...
    error_control = ACTUAL_DATA.inNormalPlane - DESIRED_DATA.inNormalPlane
    error_norm = np.linalg.norm(error_control)

    # L = ImageJacobian(ACTUAL_DATA.inNormalPlane.T, np.ones((POINTS.shape[0], 1)), 1)
    # L_inv = np.linalg.pinv(L)

    U = kp * L_inv @ error_control.reshape(-1, 1)
    print(
    ...
    if np.linalg.norm(error_control) < 0.01:
        END = t
        break
