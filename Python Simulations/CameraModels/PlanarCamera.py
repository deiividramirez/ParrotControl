...
class PlanarCamera:
    ...
    def __init__(self):
        ...
        self.focal = 0.002
        self.rho = np.array([1.0e-5, 1.0e-5])  # pixel dimensions
        self.pp = np.array([self.width / 2, self.height / 2])  # principal point
        self.K = np.array(
            [
                [self.focal / self.rho[0], 0.0, self.pp[0]],
    ...
    def set_position(self, x, y, z, rot_x, rot_y, rot_z):
        ...
        self.P = np.dot(
            self.K,
            np.concatenate((self.R, np.dot(-self.R, self.t).reshape(3, 1)), axis=1),
        )
    ...
    def projection(self, Xs, n):
        ...
        if self.noise:
            noise_matrix = np.random.normal(0.0, self.sigma_noise, size=points.shape)
            points += noise_matrix
    ...
    def euler_to_rotmat(self, rot_x, rot_y, rot_z):
        ...
        Rx = np.array(
            [
                [1.0, 0.0, 0.0],
                [0.0, cos(rot_x), -sin(rot_x)],
                [0.0, sin(rot_x), cos(rot_x)],
            ]
        )
        Ry = np.array(
            [
                [cos(rot_y), 0.0, sin(rot_y)],
                [0.0, 1.0, 0.0],
    ...
    def draw_frame(
        self, ax, c="red", scale=2.0, _scale_x=None, _scale_y=None, _scale_z=None
    ):
        """
        function: draw_frame
        ...
        Oc = scale * np.array([[0.0, 0, 0]]).T
        Xc = scale_x * np.array([[1.0, 0, 0]]).T
        Yc = scale_y * np.array([[0.0, 1, 0]]).T
        Zc = scale_z * np.array([[0.0, 0, 1]]).T
        Ri2w = R
        Oc1 = Ri2w.dot(Oc) + t
        Xc1 = Ri2w.dot(Xc) + t
        Yc1 = Ri2w.dot(Yc) + t
