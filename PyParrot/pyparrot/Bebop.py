...
class Bebop:
    def __init__(self, drone_type="Bebop2", ip_address=None):
        ...
        self.drone_connection = WifiConnection(
            self, drone_type=drone_type, ip_address=ip_address
        )
    ...
    def safe_takeoff(self, timeout):
        ...
        while self.sensors.flying_state != "takingoff" and (
    ...
    def safe_land(self, timeout):
        ...
        while self.sensors.flying_state not in ("landing", "landed") and (
            actualTime := (time.time() - start_time < timeout)
        ):
            if self.sensors.flying_state == "emergency":
                return -1
        ...
        while self.sensors.flying_state != "landed" and (
            actualTime := (time.time() - start_time < timeout)
        ):
            if self.sensors.flying_state == "emergency":
                return -1
    ...
    def flip(self, direction):
        ...
        if fixed_direction not in ("front", "back", "right", "left"):
            print(
                "Error: %s is not a valid direction.  Must be one of %s" % direction,
    ...
    def move_relative(self, dx, dy, dz, dradians):
        ...
        # send the command
        print("Sending move_relative command")
    ...
    def set_video_recording(self, mode):
        ...
        if mode not in ("quality", "time"):
            print(
                "Error: %s is not valid value. The value must be : quality, time" % mode
            )
            print("Ignoring command and returning")
            return
